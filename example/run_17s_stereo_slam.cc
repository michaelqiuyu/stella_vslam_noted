...
void mono_tracking(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    const seventeen_department sequence(sequence_dir_path);
...
void stereo_tracking(const std::shared_ptr<stella_vslam::system>& slam,
    ...
    // use mask
    const cv::Mat left_mask = cv::imread("/home/xiongchao/workspace/leador/project/vslam/dataset/17s_stereo/left_mask.png", cv::IMREAD_GRAYSCALE);
    const cv::Mat right_mask = cv::imread("/home/xiongchao/workspace/leador/project/vslam/dataset/17s_stereo/right_mask.png", cv::IMREAD_GRAYSCALE);
    ...
    // run the slam in another thread
    std::thread thread([&]() {
        for (unsigned int i = 1000; i < frames.size(); ++i) {
            // wait until the loop BA is finished
            if (wait_loop_ba) {
                while (slam->loop_BA_is_running() || !slam->mapping_module_is_enabled()) {
                    std::this_thread::sleep_for(std::chrono::milliseconds(100));
...
#if 1
            cv::imwrite("/home/xiongchao/图片/mask/left_image.png", left_img_rect);
            cv::imwrite("/home/xiongchao/图片/mask/right_image.png", right_img_rect);
#endif
            ...
            if (i % frame_skip == 0) {
                // input the current frame and estimate the camera pose
                slam->feed_stereo_frame(left_img_rect, right_img_rect, frame.timestamp_, left_mask, right_mask);
            }
